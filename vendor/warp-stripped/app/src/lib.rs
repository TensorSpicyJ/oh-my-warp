...
fn initialize_app(
    ...
    if user_is_logged_in && ChannelState::official_cloud_services_enabled() {
    ...
    } else if ChannelState::official_cloud_services_enabled() {
        ...
        log::info!("Skipping logged-out cloud telemetry in omw/local mode");
    }
...
/// Runtime flags that are tied to Warp-operated accounts, billing, Drive, cloud agents, or Oz.
...
const OMW_LOCAL_DISABLED_FLAGS: &[FeatureFlag] = &[
    ...
    FeatureFlag::AIBlockOverflowMenu,
    ...
    FeatureFlag::BuildPlanAutoReloadBannerToggle,
    FeatureFlag::BuildPlanAutoReloadPostPurchaseModal,
...
pub fn enabled_features() -> HashSet<FeatureFlag> {
    ...
    if ChannelState::omw_local_mode() {
        for flag in OMW_LOCAL_DISABLED_FLAGS {
            flags.remove(flag);
        }
    }
