...
fn is_official_cloud_workspace_action(action: &WorkspaceAction) -> bool {
    ...
    matches!(
        ...
        action,
        ShowSettingsPage(section) if is_omw_local_hidden_settings_section(Some(*section))
    ) || matches!(
        action,
        ShowSettingsPageWithSearch { section, .. }
            if is_omw_local_hidden_settings_section(*section)
    )
}
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        if !ChannelState::official_cloud_services_enabled()
            && is_official_cloud_workspace_action(action)
        {
            log::debug!("Ignoring official-cloud workspace action in omw/local mode: {action:?}");
            return;
        }
